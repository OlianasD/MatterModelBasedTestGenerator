...
def get_paths(xmlfile, lang, target_fnam, app_name):
    init_state = "init"
    if (lang == '-j'):
        header, model_lang_map, test_lang_cfg = get_java_config(target_fnam, app_name)
    elif (lang == '-p'):
        header, model_lang_map, test_lang_cfg = get_python_config()


    with open('config/'+app_name+'/wrappers.json', 'r') as f:
        map_dict = json.load(f)
    with open('config/'+app_name+'/implementations.json', 'r') as f:
    ...
    #paths.sort(key=len)
    print_paths(paths)
    #print('Minimizing test suite...')
    #prevlen = len(paths)
    #paths = minimize(paths)
    #print('Removed '+str(prevlen-len(paths))+' paths')
    print_paths(paths)
...
def minimize(paths):
    ...
    for i in range(0, len(paths)):
        ...
        for j in range(i+1, len(paths)):
            if(check_included(paths[i], paths[j])):
                print('Path '+str(i+1)+' is included in path '+str(j+1))
                keep = False
                break
        if keep:
            res.append(paths[i])
...
if __name__ == '__main__':
    get_paths(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])