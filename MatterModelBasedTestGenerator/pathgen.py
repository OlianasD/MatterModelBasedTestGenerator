...
def build_tests(paths, named_states, testfnam, testnamestr, glob_pre, pre, post, glob_post):
    ...
    for path in paths:
        ...
        for currtrans in path:
            code = json.loads(currtrans[2].replace('\"','\\"').replace('\'', '\"'))
            edgeprintableinfo = '//transition '+str(n_trans)+': '+currtrans[0]+' -> '+currtrans[1]+'   '
            if('event' in code):
                edgeprintableinfo += code['event']['code']+'\n'
            else:
                edgeprintableinfo += '\n'
            testfile.write(edgeprintableinfo)
            # run event
