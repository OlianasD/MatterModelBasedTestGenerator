...
def parse_assignment(assignment_matcher, map_dict, code, regex, lang_map, init_map, objects):
    target = assignment_matcher.group('target')
    ...
    callmatch = re.fullmatch(regex['call'], rightside)
    if (opmatch):
        ...
        uml_params = callmatch.group('params')
        ...
        varname = callmatch.group('actor')
        clsname = objects[varname]
        curr_transform = map_dict[clsname][callmatch.group('function')]
        res = curr_transform['function'] + '(' + uml_params
        if (curr_transform['params'] != '' and uml_params != ''):
            res += ','
        res += (curr_transform['params'] + ')')
        target_code = varname+"."+res
        actual = target +' '+lang_map[oper]+' '+target_code
        return {'type' : 'initialization', 'code': target +' '+lang_map[oper]+' '+target_code}
